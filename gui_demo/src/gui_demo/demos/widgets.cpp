...
DEFINE_DEMO(
    ...
    void do_ui(gui_context& ctx)
    {
        ...
        do_radio_button(ctx, make_radio_accessor(selection, in(0)), text("An option"));
        do_radio_button(ctx, make_radio_accessor(selection, in(1)),
            text("An option with a long description - Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris vulputate lectus vel odio euismod in dapibus justo mattis. Vestibulum semper pellentesque ultrices."));
        do_radio_button(ctx, make_radio_accessor(selection, in(2)), text("Another option"));
    })
